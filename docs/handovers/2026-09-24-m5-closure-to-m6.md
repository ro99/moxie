...
|---|---|---|---|---|
| M6.1 | Device-resident layer chains, batched/grouped expert kernels, fused dequant/activation/reduction, output-minimizing transfers | **Active** | luna, slice 1 | [0076](../tasks/0076-m6-device-kv-append.md) **accepted** 2026-09-24 (`1f64a92`, `0f5363f`): no per-layer K/V device→host→device copy; decode host workspace 128 → 64 bytes on `gemma-a`; outputs bit-identical; no timing claim. Remaining in slice 1: per-operation `settle` in `PagedAttentionRun`, the per-Rope `synchronize`, the routed-expert route readback, per-step module load |
