...
| M6.3 | Measured transfer overlap, bounded read-ahead, optional hot-expert tiers and route prediction, against no-overlap baselines | **Active** | luna, [0089](../tasks/0089-m6-pinned-transfer-probe.md) | First measure pageable vs pinned transfers and copy/compute overlap on this machine (document 03, R12); then admit bounded pinned staging and add KV-streaming read-ahead against the no-prefetch baseline. Carries M5's pipeline microbatch overlap and pinned transfers |
