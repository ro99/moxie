...
| 2 | Quantized weights in the dense step | **Re-scoped (coordinator, 2026-09-24)** from "device routing and grouped experts": device experts already route on the device (`DENSE_ROUTE`); the only route readback is the host-owned-expert join, whose overlap is M6.3. What the dense step lacks is any quantized weight. [0080](../tasks/0080-m6-affine-linear-admitted-in-dense-plans.md) **accepted** 2026-09-24: plans admit affine INT4/INT8 `Linear` weights through a planner format map, and the dense `launch` checks symbol identity. [0081](../tasks/0081-m6-affine-linear-in-the-dense-step.md) **active**: the single-GPU step launches them. Grouped expert kernels and quantized experts in the graph follow only if the exit benchmarks need them | M6.1 fused dequantization; prerequisite of slice 7's quantized checkpoint benchmarks |
