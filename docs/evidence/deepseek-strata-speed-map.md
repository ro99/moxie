...
**Moxie home values:**
- **present**: the cited Moxie code does what the Strata technique does,
  with the same mechanism and effect. A related or weaker mechanism is
  **planned** or **gap**, and the Note says what differs.
...
| 13 | One MoE command per GPU, with every GPU enqueued before any result is collected | decode | `E0006-…-baseline-handoff:77` "1.8018641620x"; `:81` syncs "161,846 \| 62,285" | accepted | Generic | **planned**: M6.1 batched/grouped expert kernels (`06:100`) | Moxie launches two kernels per expert group (`crates/moxie-kernels/cuda/expert_mlp.cu:1-7`), not one MoE command per GPU. Cross-GPU concurrency comes from rank threads (task 0062). |
...
| 31 | Decode: routed experts on NUMA-local CPU, shared expert on GPU, joined through a stream-ordered host callback and a device join kernel | decode | `E0069:445-447` "Complete MoE is 164.387 ms … routed-GPU arm had spent 396.429 ms … Routed-expert weight H2D falls from 1,377,042,432 bytes to zero"; `E0069:543-545`; `deepseek_executor.cpp:39-50`; `c:bce98dd` | accepted (production bundle) | Generic | **gap** | Moxie runs host experts synchronously after `stream.synchronize()` (`crates/moxie-executor/src/dense.rs:1933`). `launch_host_func` is used only by the KV read-ahead (`paged_attention.rs:4835`). Host experts themselves exist (`host_experts.rs:56`, task 0068). |
...
| 37 | Group a page's rows per expert on the CPU (decode each weight tile once for 4 rows) | prefill | `E0094:47` "2.315x"; `E0095:145` "2.180x"; `c:b28219d` "1.204x"; superseded by GPU prefill experts (`c:fe8f9e5`) | superseded | Generic | **planned**: M6.1 batched/grouped expert kernels (`06:100`) | `expert_group_affine` (`cpu_expert.rs:179`) loops rows outermost (`:314-339`), so each weight tile is re-read for every row. |
...
| 40 | Batched multi-query attention: pack a page's KV union once, with a per-query visibility mask | both | `E0030:89` "1.308x on prefill and 1.0079x on decode"; `:75` FlashAttention calls "21,973 \| 344"; `:97-98` "forced dispatch beats hybrid by 1.399x on prefill" | accepted | Generic | **gap** | `paged_attention.cu` scans pages for each query row and head. Nothing packs a shared KV union once. `04:21` names FlashAttention-2 as a candidate, but no roadmap item covers it. |
...
| # | Technique | Phase | Strata evidence | Outcome | Scope | Moxie home | Note |
|---|---|---|---|---|---|---|---|
| 67 | Rank-sharded TP2 ownership with a slice loader that reads only this rank's byte ranges | load | `E0073:328` "6,223,273,412 per rank"; `:327` "peak loader temporary \| 529,530,880 per rank"; `deepseek_rank_local_weights.hpp:82-89`; `rl.md:66-75` | accepted | Generic | **gap** | `lower_tensor_parallel` (`crates/moxie-plan/src/tensor_parallel.rs:498` at HEAD `60fc509`) lowers graph slices. `CanonicalSource::read_chunk` (`crates/moxie-executor/src/residency.rs:255-264`) requires a whole-tensor range, so nothing loads only a rank's bytes. |
...
| 71 | Vocab-parallel embedding all-reduce and LM-head all-gather | decode | `E0074:332` "0.040 / 0.040–0.041 ms"; `:324` "0.088 / 0.081–0.091 ms" | accepted | Generic | **gap** | The LM-head all-gather is present (`crates/moxie-executor/src/tensor_parallel.rs:359`). There is no vocab-parallel embedding all-reduce. |
...
| 75 | Cross-device grouped ownership through fixed pinned staging and one reusable event | decode | `E0069:1441` "synchronization calls \| 197 \| 132 \| -65 (-33.0%)"; `:1449` "-2.516 ms single-arm" | accepted | Generic | **planned**: M6.3 pinned transfers (handover line 120; `03:51`) | M5 transfers and `Lease` (`lease.rs:342`) allocate no fixed pinned command slots. |
...
| 83 | Attention preparation on four streams (Q, KV, MLA compressor, indexer compressor) | decode | `E0069:1536` "-5.275423 ms (-13.53%)"; `:1541` decode "-18.906416 ms (-10.71%)" | accepted | Generic | **planned**: M6.3 measured overlap and bounded preparation (`06:102`) | Stream primitives exist (`driver.rs:740`). |
...
| 86 | Reusable outer decode CUDA graph | decode | `E0070:67-68` "0.388 ms/token (0.25%) slower"; external claim `E0050:85-86` not measured | rejected | Generic | **planned**: M6.2 stable decode graph capture (`06:101`) | Only piecewise capture exists (`crates/moxie-executor/src/dense.rs:379`, task 0085). Attention stays eager, and there is no full-step capture. |
...
| 89 | Inline BF16 encode helpers so host attention passes vectorize | prefill | `E0113:58` "27.659 s \| 15.472 s"; `:60` attention total "92.520 s \| 78.139 s"; `c:c7bc93d`; truncate-only proposed `E0113:95-96` | accepted | Generic | **gap** | Moxie has no host attention pass to tune. Attention runs on the device (`paged_attention.cu:85`), which in Strata superseded this technique. |
| 90 | One pool task per row instead of per (row, head) | prefill | `runtime_attention.inc.cpp:1619-1622` "1.86 million tasks … 11.2 s in pool overhead" | accepted | Generic | **gap** | Moxie has no host attention pass to tune. Attention runs on the device (`paged_attention.cu:85`), which in Strata superseded this technique. |
| 91 | Host attention heads split across 28 persistent affinity-pinned workers | decode | `E0007:87-88` "3.0227143404 \| 4.3789600899", "1.4486847240x" | accepted, later superseded by CUDA attention | Generic | **gap** | Moxie has no host attention pass to tune. Attention runs on the device (`paged_attention.cu:85`), which in Strata superseded this technique. |
...
| 93 | Fixed per-command pinned staging slots for queued async H2D | decode | `E0091:164` "43 x 16 KiB = 688 KiB of pinned host memory per device"; `E0069:1805-1806` "5,636,096 pinned bytes"; `rl.md:98-119` | accepted | Generic | **planned**: M6.3 pinned transfers (handover line 120; `03:51`) | `Lease` (`lease.rs:342`) retains buffers by event, but there are no fixed pinned command slots. `PinnedHostBuffer` is not used in the TP or grouped paths. |
...
| 100 | Expert-major MoE page: each distinct expert read once per page, 8-row tile kernels | prefill | `E0056:97` "1.262"; `:104` "70,483 \| 1,290 \| 54.6x fewer"; `c:ddb7356` "1.262x … MoE 1.889x" | accepted | Generic | **present**: `GroupedRun` groups a chunk's rows per expert (`grouped.rs:1162`; `04:43`) | Transfer-wise one read per expert. Each thread reads its own weights, with no tile reuse across rows (`expert_mlp.cu:182-195`). |
...
| 107 | Router run on device from the resident output; routing and CPU-MoE input prepared inside the host callback | decode | `E0069:1213` syncs "218 \| 197"; `E0069:1649` "132 \| 89 \| -43 (-32.6%)" | accepted | Generic | **gap** | Routing on device is present (`routed_ops.cu:41`, task 0035). In-callback preparation is not, because Moxie has no stream-ordered host callback (row 31). |
...
| 110 | Charge communicator and first-warmup memory into the admission reserve | load | `E0083:129` "communicators initialized \| 391,249,920"; `:130` "first warmup chain complete \| 428,998,656" | accepted | Generic | **gap** | Graph-pool bounds (`chain.rs:220-223`, task 0086) charge captured graphs, not communicators or first-warmup memory. Moxie has no NCCL communicator. |
...
**Counts (117 rows).** Changed in review round 1: see the task's Result.
...
- **No place in Moxie's spec or code, rejected in Strata:** 10, 30, 69, 74,
  104.
- **No place, host-attention tuning:** 89, 90, 91. Moxie attends on the
  device, so there is no host pass to tune.
- **No place, accepted in Strata:** 4, 5, 24, 31, 32, 33, 34, 35, 40, 43, 49,
  61, 67, 71, 82, 87, 92, 94, 107, 110. By area:
  - **Host expert execution:** 31, 32, 33, 34, 35, 92, 107. There is no
    stream-ordered host callback, no host worker pool (the kernel is
    single-threaded), and host buffers are allocated per call. Strata's decode
    rate is set by this host DRAM expert read (`ds.md:8-10`, `ds.md:154-155`).
  - **Loading:** 4, 5, 67 (no parallel reader, no warm-up overlap, no
    rank-slice loader).
  - **KV and page bookkeeping:** 24, 43, 49 (sorting eviction scan, KV
    capacity allocated up front, per-publish page-table allocation).
  - **Attention:** 40, 61 (no shared-KV multi-query packing, no bounded
    sub-chunked workspace).
  - **Multi-GPU and admission:** 71, 110 (no vocab-parallel embedding
    all-reduce; communicator and warm-up memory not charged).
  - **Kernels and launch:** 82, 87, 94 (replay guard, host callbacks inside
    a graph, serial RMS kernel).
