...
**Moxie home values:**
- **present**: Moxie code or an accepted task provides the mechanism.
- **planned**: a roadmap item names it.
- **gap**: nothing in Moxie's spec or code gives it a place, or policy forbids
  it.
...
|---|---|---|---|---|---|---|---|
| 1 | Routed experts live in host DRAM; VRAM holds the dense spine plus an expert cache; no NVMe reads in steady state | both | `rt.md:111-120` "Main routed-expert source extents \| 147,169,738,752", "Steady-state NVMe \| 0"; `ds.md:7-10` ("147 GB against 48 GB of aggregate VRAM", line 8) | accepted | Generic | **present**: `crates/moxie-memory/src/residency.rs:1282` `ResidencyAuthority` (task 0020) | Never exercised at 147 GB. |
| 2 | One capacity-bounded CUDA weight arena per GPU instead of per-weight `cudaMalloc`/`cudaFree` | both | `E0011:34` "10.13% decode throughput improvement"; `:45` decode allocations "7,716 \| 0"; `:46` prefill wall "4.5967 s \| 3.2035 s \| -30.31%" | accepted | Generic | **present**: `crates/moxie-executor/src/arena.rs:298` `DeviceArena` (task 0010); `crates/moxie-memory/src/ledger.rs:355` | |
...
| 8 | GPU prefill streams experts from the host arena's copy, not from the checkpoint files | prefill | `E0096:76` "69.9 GB of reads becomes 0.57 GB"; `:106-107` "101.05 s" → "93.46 s"; rejected: rebuild canonical bytes on the host `E0096:64` "1.21 GB/s single-threaded, or 58 s of rebuild per page" | accepted | Generic | **present**: `ResidencyAuthority` host cache is the upload source (`crates/moxie-executor/src/residency.rs:381` `drain_reads`) | Strata's transformed layout is FP4-specific (row 7). |
| 9 | NUMA: bind each expert-arena shard to its node before first touch; node-local first touch for CPU reads | load, decode | `E0058:49-50` "bind … 56.7 GB/s", "no-bind … 22.9 GB/s"; `E0050:141` "both nodes, node-local \| 28 \| 76.3"; `deepseek_checkpoint.cpp:377-391`; `E0106:74-75`. Rejected for DMA: `E0026:120-121` "NUMA binding alone is 1.02x" | accepted | Generic | **present**: `crates/moxie-executor/src/grouped.rs:387` mbind placement (task 0021); `crates/moxie-host/src/numa.rs` | Pinned-buffer NUMA placement is uncontrolled (task 0089). |
...
| 13 | One MoE command per GPU, with every GPU enqueued before any result is collected | decode | `E0006-…-baseline-handoff:77` "1.8018641620x"; `:81` syncs "161,846 \| 62,285" | accepted | Generic | **present**: `crates/moxie-executor/src/grouped.rs:1162` `GroupedRun`, `grouped_device.rs:51` (tasks 0021, 0035) | |
...
| 22 | Expert prefetch driven by DSpark draft routes | decode | `E0029:187-189` "upload wait was 4.840 seconds, while the observed draft alone cost 4.912 seconds" | rejected | DeepSeek-only (DSpark) | **planned**: M6.3 route prediction (`06:102`) | |
| 23 | Dynamic LRU expert cache / frequency-aware placement (projected only) | decode | `E0124:112` "3090s LRU 17.7 GB only … 13.26" (simulation); `E0050:368-369` "83% → 90% hit would cut host bytes from 586 MB to 345 MB per token" (projection) | proposed | Generic | **present**: demand LRU in `ResidencyAuthority` (`residency.rs:1282`, task 0020) | |
| 24 | O(1) eviction through intrusive recency lists (was a full scan per miss) | decode | `E0057:48` "14.3 ms/step"; `deepseek_runtime.cpp:463-468` "walked roughly 670,000 hash nodes a step"; `E0120:502-503` | accepted | Generic | **present**: deterministic eviction in `ResidencyAuthority` (`residency.rs:2781-2790`) | Moxie's per-miss eviction cost is not measured. |
...
| 28 | Experts spread across devices round-robin (centralized mode) | both | `deepseek_runtime.cpp:1864-1866`; not measured | accepted | Generic | **present**: expert-partitioned plans (task 0066; `crates/moxie-plan/src/expert.rs:84`) | |
...
| 30 | Divert cache-miss experts to the host kernel | decode | `E0051:344` "0.76x. Rejected."; write-behind `E0051:244` "4.15x regression"; `deepseek_host_expert.hpp:11-15` | rejected | Generic | **present**: mixed CPU/GPU candidates (`03:54`; `expert.rs:84`) | |
| 31 | Decode: routed experts on NUMA-local CPU, shared expert on GPU, joined through a stream-ordered host callback and a device join kernel | decode | `E0069:445-447` "Complete MoE is 164.387 ms … routed-GPU arm had spent 396.429 ms … Routed-expert weight H2D falls from 1,377,042,432 bytes to zero"; `E0069:543-545`; `deepseek_executor.cpp:39-50`; `c:bce98dd` | accepted (production bundle) | Generic | **present**: `crates/moxie-plan/src/host_experts.rs:56` (tasks 0064, 0068); `launch_host_func` (`driver.rs:863`) | Never measured at DeepSeek scale. |
...
| 37 | Group a page's rows per expert on the CPU (decode each weight tile once for 4 rows) | prefill | `E0094:47` "2.315x"; `E0095:145` "2.180x"; `c:b28219d` "1.204x"; superseded by GPU prefill experts (`c:fe8f9e5`) | superseded | Generic | **present**: `expert_group_bf16` / `expert_group_affine` (`cpu_expert.rs:152,179`) take a row group | |
...
| # | Technique | Phase | Strata evidence | Outcome | Scope | Moxie home | Note |
|---|---|---|---|---|---|---|---|
| 39 | Shared CUDA FlashAttention backend: a campaign covering the crossover, block-parallel softmax and forced dispatch | both | Accepted: block-parallel softmax `E0057:65` "18.2 → 9.7 ms/step"; forced dispatch in the bundle (`deepseek_runtime.hpp:72-75`). Rejected: as default `E0014:28` "`0.520x`"; production flash decode `E0015:103` "1.288 \| 1.148 \| -10.9%"; tiled F32/F64 online softmax on numerics `E0013:82-85`; 256-row hybrid `E0027:89-90` "1.024x" / "1.010x" (superseded by row 40); `fa.md:92-98` | opt-in → forced in the bundle | Generic | **present**: `crates/moxie-kernels/cuda/paged_attention.cu:85` (task 0037, ADR 0032) | Moxie's kernel has no tensor cores and no tuning (its header). |
| 40 | Batched multi-query attention: pack a page's KV union once, with a per-query visibility mask | both | `E0030:89` "1.308x on prefill and 1.0079x on decode"; `:75` FlashAttention calls "21,973 \| 344"; `:97-98` "forced dispatch beats hybrid by 1.399x on prefill" | accepted | Generic | **present**: one kernel serves prefill, chunk and decode rows (`paged_attention.cu` header; task 0037) | |
| 41 | One pinned H2D and one D2H per attention call | decode | `E0015:59-60` "from about 3.07 ms to 2.00 ms"; `fa.md:50-54` | accepted (opt-in path) | Generic | **present**: attention reads device-resident pages (`attend_into`, `crates/moxie-executor/src/paged_attention.rs`; task 0082) | |
...
| 48 | O(1) physical KV block lookup (uniform guess, then binary search, then scan), on host and device | decode | `deepseek_runtime.cpp:1554-1560` "measured 2,889.637 ms per decoded token across the 43 layers" before; `backend_model_kernels.cuh:1167-1182` | accepted | Generic | **present**: page tables published to the kernel (`publish_page_table_deferred`, `paged_attention.rs`; task 0082) | |
| 49 | Reuse block-table buffers (no allocation in the timed path) | decode | `deepseek_runtime.cpp:1782-1787` "cost about 8.1 ms/token" | accepted | Generic | **present**: admitted arenas and no growth across turns (task 0088) | |
| 50 | Positional page numbering with lazy first-touch leasing; cache the positional page prefix | decode | `c:4115d69` "161.338 ms/token" → "159.673 ms/token"; `c:2cd2f20` "it removes 0.061 ms" | accepted | DeepSeek-only (compressed KV) | **planned**: M7 DeepSeek (`06:120`) | |
| 51 | Page-width-aware KV admission and an explicit host-KV budget | prefill | `E0119:66-67` "prefill 197.5 s / 19.47 tok/s after" (3,845 tokens; failed before); `E0110:72` host peak "21,729,792 B \| 81,616,576 B"; `:77-78` prior budget "about 5.7x below" | accepted | Generic (sliding window plus compressed stream) | **present**: `Ledger::admit` (`ledger.rs:355`); `Retention::Window` (`crates/moxie-state/src/paged.rs:28-40`) | |
...
| 55 | Physical E4M3 lightning index with radix top-k select (three 16-bit passes) | decode | `c:bfd59ed` "66.3 ms/token at 1M, from 842 ms"; `rl.md:730` "Scoring, `2.15x`"; `rl.md:739` "Radix pivot, `3.01x`" | accepted | DeepSeek-only (indexer) | **planned**: M7 DeepSeek (`06:120`) | Index dtype falls under ADR 0019 item 1. |
...
| 58 | Shard candidate scoring across ranks | decode | `rl.md:813-820` "a strict lower bound of `156.656105 ms/token`" | rejected | DeepSeek-only (indexer) | **planned**: M7 DeepSeek rank-local semantics (`06:120`) | |
...
| 60 | Row-batched physical prefill attention: gather pages once into a flat workspace, one score op per page | prefill | `E0098:88-92` "paged-attention kernel launches \| 553,109 \| 1,677", "attention score \| 18.324 s \| 13.945 s" (gate failed); promoted inside `E0107:169-171`. Rejected: page-set hoist `E0097:82` "candidate +1.9%"; prepared `index_select` `E0099:125` "14.159 s"; gather-once `E0111:181-184` | accepted (in the 0107 stack) | DeepSeek-only (sparse candidates) | **planned**: M7 DeepSeek (`06:120`) | |
| 61 | Page attention bounded by workspace: exact-extent 384 MiB cap, binary-searched query sub-chunks | prefill | `E0099:167-168` "226,492,416 bytes less on each GPU"; `E0109:90` "+1.912258 s" (functional reject; code active per `E0110:62-63`) | accepted | Generic | **present**: `prefill_chunks` buckets (`crates/moxie-plan/src/selected.rs:170`, task 0087); admitted workspaces (task 0086) | |
...
| 63 | Hold the RoPE-decoded attention result as BF16 | prefill | `c:92b6163` "844.56 -> 681.37 MB … no timing win is claimed" | accepted | Generic | **present**: 16-bit activations (ADR 0003 line 10) | |
| 64 | Exact physical-page attention output (materialize, cuBLAS BMM, reference finish); native WMMA page attention rejected | decode | `E0068:269-271` "0.058368", "0.031744", "0.057344" ms; rejected `E0066:87-88` (correctness) | accepted | DeepSeek-only (sparse MLA) | **planned**: M7 DeepSeek (`06:120`) | Moxie's MLA is host-only (task 0040). |
...
| 66 | Rank 0 alone runs the compressor and publishes the row bytes to both ranks | decode | `rl.md:179-186`; `deepseek_rank_local_weights.cpp:286` "would add roughly 0.6 GiB" | accepted | DeepSeek-only (compressor) | **planned**: M7 DeepSeek (`06:120`) | |

### 2.3 Multi-GPU (rank-local, TP, transfers)

| # | Technique | Phase | Strata evidence | Outcome | Scope | Moxie home | Note |
|---|---|---|---|---|---|---|---|
| 67 | Rank-sharded TP2 ownership with a slice loader that reads only this rank's byte ranges | load | `E0073:328` "6,223,273,412 per rank"; `:327` "peak loader temporary \| 529,530,880 per rank"; `deepseek_rank_local_weights.hpp:82-89`; `rl.md:66-75` | accepted | Generic | **present**: `crates/moxie-plan/src/tensor_parallel.rs:498` `lower_tensor_parallel` (at HEAD `60fc509`) (tasks 0057, 0060) | |
| 68 | FP32 all-reduce (NCCL over SHM, no P2P), 86 per forward, BF16 publication | decode | `E0074:237` "FP32 NCCL sum → BF16 \| 86 \| … \| 2.845 / 2.817–2.884 ms"; `c:be720ed` "all 86 per token cost 1.36 ms -- 3.3%" | accepted | Generic | **present**: `moxie_tp_reduce_f32_v1` (`crates/moxie-kernels/cuda/dense_ops.cu:73`) over peer copies (ADR 0036) | Moxie uses no NCCL. Its 86-per-forward cost is unmeasured. |
| 69 | Host-staged TP reduction (pinned D2H, host sum, H2D) | decode | `E0074:52` "11.275 ms" (serialized); `:145` "25.997 ms" | rejected | Generic | **present**: declared host-staged transfers (M5; `crates/moxie-executor/src/tensor_parallel.rs:281`) | |
...
| 71 | Vocab-parallel embedding all-reduce and LM-head all-gather | decode | `E0074:332` "0.040 / 0.040–0.041 ms"; `:324` "0.088 / 0.081–0.091 ms" | accepted | Generic | **present**: `all_gather` (`crates/moxie-executor/src/tensor_parallel.rs:359`) | |
...
| 74 | The RTX 5060 Ti as a third TP rank | decode | `c:62786da` "0051 measured that at 0.79x"; `ds.md:233` | rejected | Generic | **present**: heterogeneous three-GPU pipeline (task 0071) and plan comparison (task 0074) | |
| 75 | Cross-device grouped ownership through fixed pinned staging and one reusable event | decode | `E0069:1441` "synchronization calls \| 197 \| 132 \| -65 (-33.0%)"; `:1449` "-2.516 ms single-arm" | accepted | Generic | **present**: declared host-staged transfers and peer-copy grants (M5; `driver.rs:1378`) | |
...
| 78 | Coalesced GPU expert kernel over transformed shards (a warp owns a 32-row block); row tile 8 → 32 | prefill | `E0096:80-84` "2.87x slower" with canonical addressing, "from 21.67 s to 4.02 s"; `:108` row tile 32 "86.25 s"; `:96-97` tile 64 "spills the accumulators"; `backend_kernels.cuh:3620` | accepted | Generic (FP4) | **gap**: FP4 weights not admitted (ADR 0003 line 10) | Bought 2.87x on the prefill expert kernel. Moxie's `expert_mlp.cu` has no coalescing study. |
...
| 83 | Attention preparation on four streams (Q, KV, MLA compressor, indexer compressor) | decode | `E0069:1536` "-5.275423 ms (-13.53%)"; `:1541` decode "-18.906416 ms (-10.71%)" | accepted | Generic | **planned**: M6.1 device-resident layer chains (`06:100`) | Stream primitives exist (`driver.rs:740`). |
...
| 86 | Reusable outer decode CUDA graph | decode | `E0070:67-68` "0.388 ms/token (0.25%) slower"; external claim `E0050:85-86` not measured | rejected | Generic | **present**: piecewise decode capture (`crates/moxie-executor/src/dense.rs:379`, task 0085); graph memory admitted (task 0086) | |
| 87 | CUDA graph with 43 host-function CPU-MoE callbacks (envelope probe) | decode | `E0065:91` "2.318336" → "2.182144" ms envelope median | accepted (prerequisite) | Generic | **planned**: M6.2 graph capture (`06:101`) | |
...
| 89 | Inline BF16 encode helpers so host attention passes vectorize | prefill | `E0113:58` "27.659 s \| 15.472 s"; `:60` attention total "92.520 s \| 78.139 s"; `c:c7bc93d`; truncate-only proposed `E0113:95-96` | accepted | Generic | **present**: attention runs on the device (`paged_attention.cu:85`); there is no host pass | |
| 90 | One pool task per row instead of per (row, head) | prefill | `runtime_attention.inc.cpp:1619-1622` "1.86 million tasks … 11.2 s in pool overhead" | accepted | Generic | **present**: device attention, as row 89 | |
| 91 | Host attention heads split across 28 persistent affinity-pinned workers | decode | `E0007:87-88` "3.0227143404 \| 4.3789600899", "1.4486847240x" | accepted, later superseded by CUDA attention | Generic | **present**: device attention, as row 89 | |
| 92 | Persistent, uninitialized page-projection scratch (`make_unique_for_overwrite`) | prefill | `E0111:130-131` "from 8.801146 to 0.000274 s"; `:142-143` "a 3.979124 s or 4.1% reduction"; adverse D2H `:133` | accepted | Generic | **present**: persistent admitted workspaces (`arena.rs:298`; task 0086) | |
| 93 | Fixed per-command pinned staging slots for queued async H2D | decode | `E0091:164` "43 x 16 KiB = 688 KiB of pinned host memory per device"; `E0069:1805-1806` "5,636,096 pinned bytes"; `rl.md:98-119` | accepted | Generic | **present**: event-retained leases (`crates/moxie-executor/src/lease.rs:342`, task 0009) | |
| 94 | Widen `<<<1,1>>>` attention-prep norm kernels while keeping the FP64 order | decode | `rl.md:450-454` "`dsv4_query_rank_norm <<<1, 1>>>` 240.6 us … `5.92x`, zero bit mismatches"; `rl.md:404` "111.177092 ms" | accepted | Generic | **present**: grouped RMS kernel (`crates/moxie-kernels/cuda/dense_ops.cu:113`) | Moxie's norm launch geometry has not been timed. |

### 2.5 Prefill specifics

| # | Technique | Phase | Strata evidence | Outcome | Scope | Moxie home | Note |
|---|---|---|---|---|---|---|---|
| 95 | Bounded batched prefill pages: multi-row projections and router in one matmul per page | prefill | `E0016:107` "4.232 \| 4.619 \| +9.2%"; `:113` "264,966 \| 4,257 \| -98.4%"; `c:f41126e` "1,501 projection matmul calls … against 122,591" | accepted | Generic | **present**: chunked prefill (`selected.rs:170`, task 0087) | |
| 96 | Layer-major prefill tiling (layers outermost over a token tile) | prefill | `E0023:70` "284.16 \| 150.39 \| 1.89x"; `:71` demand H2D "4.17x less"; `runtime_generation.inc.cpp:381-390` "745,172 evictions" before | accepted | Generic | **present**: each prefill chunk runs through every layer (task 0087) | Chunk size is not tied to expert streaming. No host-streamed MoE prefill has been measured. |
| 97 | Wide prefill pages (8,192 rows) | prefill | `E0119:17-18` "64 \| 58.68 s \| 4.02 tok/s", "8192 \| 23.52 s \| 10.04 tok/s"; `c:04d7932` "a 2.50x"; earlier 512 rejected `E0056:63` "0.96x"; chat wiring `E0192:9-10` | accepted | Generic | **present**: bucket sizes (task 0087; qualified `max_input` 16,384) | |
| 98 | Page-major prefill over swappable per-row mHC slots | prefill | `E0095:143` "75.720 s \| 62.886 s \| 1.204x"; `:72` "64 slots are 6.1 MB" | accepted | DeepSeek-only (mHC) | **planned**: M7 DeepSeek mHC (`06:120`) | |
...
| 100 | Expert-major MoE page: each distinct expert read once per page, 8-row tile kernels | prefill | `E0056:97` "1.262"; `:104` "70,483 \| 1,290 \| 54.6x fewer"; `c:ddb7356` "1.262x … MoE 1.889x" | accepted | Generic | **present**: `GroupedRun` groups a chunk's rows per expert (`grouped.rs:1162`; `04:43`) | |
...
| 102 | Register (pin) the host query-page scratch for D2H | prefill | `E0194:49-50` pageable "5.6062" vs registered "3.5461" GB/s | rejected | Generic | **planned**: M6.3 pinned transfers (`03:51`) | |
| 103 | Fixed-wave (32 experts) overlap of cold expert upload with page-MoE execution | prefill | `E0193:67-68` "1.269", "0.787"; `:72` demand wait "1.402"; `08:101-105` (R15) | rejected | Generic | **planned**: M6.3 measured transfer overlap (`06:102`) | |
...
| 110 | Charge communicator and first-warmup memory into the admission reserve | load | `E0083:129` "communicators initialized \| 391,249,920"; `:130` "first warmup chain complete \| 428,998,656" | accepted | Generic | **present**: graph memory admitted (`crates/moxie-executor/src/chain.rs:221-223`, task 0086); `DeviceTier::GraphPools` | Moxie has no NCCL communicator to charge. |
...
| # | Technique | Phase | Strata evidence | Outcome | Scope | Moxie home | Note |
|---|---|---|---|---|---|---|---|
| 115 | SM clock and power operating point: memory-bound kernels are clock-insensitive, ALU-bound ones are not | both | `E0138:62-63` "Removing the clock lock was worth about 20%"; `E0137:146-147` PRMT "−1.0%" under a 250 W cap vs "−8.4%" for the older decoder; production stays locked (`E0138:173-176`) | finding | Generic | **gap** | Benchmark manifests do not record the clock or power state. |
| 116 | `CUDA_DEVICE_ORDER=PCI_BUS_ID` pinned in harness and server | other | `c:1da9bab` "13.4 GiB per rank instead of 20.3"; `ds.md:31-55` | accepted | Generic | **present**: `.cargo/config.toml`; `AGENTS.md:249` | |
| 117 | Release build for the host MoE executor | decode | `E0078:27` "unoptimized `build-stage5` measured 6.603181 GB/s"; `:32` "routed_gbps=44.769263" | accepted | Generic | **present**: release-mode timing harness (task 0078, [dense-step-timing](dense-step-timing.md)) | |
...
**Gap rows by number:**
- **Representation Moxie does not admit** (FP4/FP8 weights, W8A8 activations,
  or a KV cache below 16 bits), each citing its rule: 6, 7, 15, 18, 36, 45,
  46, 77, 78, 79, 80, 81, 114.
- **Exact-reduction rule** (ADR 0036): 70.
- **Single-user rule** (`01:12`): 76.
- **No place in Moxie's spec or code:** 4, 5, 10, 32, 33, 34, 35, 82, 104, 115.
  - Accepted in Strata: 4, 5, 32, 33, 34, 35, 82.
  - Rejected in Strata: 10, 104.
  - A finding: 115.

The accepted gaps with no place cluster in two areas:
- **Load time:** rows 4 and 5.
- **The host expert worker pool:** rows 32–35. Moxie's host expert kernel is
  single-threaded, while Strata's decode rate is set by the host DRAM expert
...
|---:|---:|---|---|---|
| 1 | 79 | "21.14x faster" query matmul device time | one prefill term | `E0105:101` |
| 2 | 5 | "about 8.6x" initialization, 191 s → 22.177 s | whole load lineage (rows 3–6) | `E0008:55-56` |
| 3 | 94 | "`5.92x`" | one norm kernel | `rl.md:454` |
| 4 | 96 | "4.17x less" prefill demand H2D bytes (and 1.89x prefill time) | prefill | `E0023:70-71` |
| 5 | 36 | "a 4.1x on the mechanism" | host FP4 expert read vs PCIe path | `E0051:169` |
| 6 | 11 | "3.33x less" demand wait (and "1.626x" decode) | decode | `E0024:82,84` |
| 7 | 55 | "Radix pivot, `3.01x`" | one indexer stage | `rl.md:739` |
| 8 | 78 | "2.87x" | prefill expert kernel | `E0096:80-84` |
