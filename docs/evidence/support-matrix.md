...
| `G-HOST-TEST` | `cargo test --workspace --locked` | host | passed, 332 unit/integration tests + 4 doctests |
...
| `G-HOST-NODRIVER` | host build with `CUDA_HOME=/nonexistent NVCC=/nonexistent`, no CUDA on `PATH` | host | passed, 332 unit/integration tests + 4 doctests, `ldd` shows no `libcuda` |
