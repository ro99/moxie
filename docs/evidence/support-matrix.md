...
| `G-HOST-TEST` | `cargo test --workspace --locked` | host | passed, 330 unit/integration tests + 3 doctests |
...
| `G-HOST-NODRIVER` | host build with `CUDA_HOME=/nonexistent NVCC=/nonexistent`, no CUDA on `PATH` | host | passed, 330 unit/integration tests + 3 doctests, `ldd` shows no `libcuda` |
...
| `G-INTERP-BF16` | `cargo test -p moxie-interp` | host | passed, 17 unit + 27 acceptance tests + 2 compile-fail doctests ([task 0003](../tasks/0003-m1-bf16-reference-interpreter.md), [task 0004](../tasks/0004-m1-state-transactions.md)) |
