...
| none | n/a | n/a | First / partial / later chunked prefill | **not implemented** | `G-HOST-TEST` (`moxie-oracles::mask`) covers the *mask* arithmetic only | M1/M4. Chunk-versus-whole parity is proven on masks, not on execution. |
