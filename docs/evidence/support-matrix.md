...
`G-PAGED-ALLOCATION` pass. After review correction `c9a4b33`, the host workspace
passes **570 tests + 8 doctests**,
...
| none | host | 32,768 stored rows; no attention | Appendable paged KV state | **host gates passed; task 0013 review correction applied, owner acceptance pending** | `G-PAGED-HOST`, `G-PAGED-ALLOCATION` | One root branch; fixed pool admitted by the shared ledger. BF16/FP16/FP32 byte preservation, partial pages and cancellation/rollback. Post-admission lineage exhaustion is typed capacity failure with complete release. Pool remains fully charged until close. No COW, dynamic growth, sliding reclamation, device attention, sampler or model support. |
