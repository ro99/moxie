...
existing sequence transactions; owner review is pending. `G-PAGED-HOST` and
`G-PAGED-ALLOCATION` pass. The host workspace now passes **569 tests + 8 doctests**,
and architecture passes **61 rejecting + 16 accepted fixtures**. Device regressions
pass **585 tests + 11 doctests**, device clippy and **39/39** aggregate real GPU
...
| `G-HOST-TEST` | `cargo test --workspace --all-targets --locked --offline`; `cargo test --workspace --doc --locked --offline` | host | passed, 569 unit/integration tests + 8 doctests after task 0013 implementation |
...
| `G-PAGED-HOST` | `cargo test -p moxie-state -p moxie-memory --locked --offline` | host | passed, task 0013; complete per-layer encoded rows across page boundaries, whole/chunk/later parity, partial acceptance/truncation, every cancellation boundary, sequence-unique transactions, admission/cleanup and explicit unsupported forks |
...
Owner acceptance is recorded in task 0012: **M1.3 complete; M1.4 active**. Task 0013 now
implements appendable host paged state bound to task 0004, with owner review pending;
...
| none | n/a | n/a | Transactional publication of a step | passed | `G-INTERP-BF16` (`moxie-state` `begin`/`commit_prefix`/`abort`, `a_step_cancelled_at_any_publication_boundary_aborts_to_exactly_where_it_started`) | Host reference only. A step's sequence-state and KV mutations commit together or restore exactly, and a transaction is append-only on both. `fork` is still identity-only (COW is M4), and task 0013 separately binds physical host pages to the same sequence journal, with owner review pending. |
| none | host | 32,768 stored rows; no attention | Appendable paged KV state | **host gates passed; task 0013 review pending** | `G-PAGED-HOST`, `G-PAGED-ALLOCATION` | One root branch; fixed pool admitted by the shared ledger. BF16/FP16/FP32 byte preservation, partial pages and cancellation/rollback. Pool remains fully charged until close. No COW, dynamic growth, sliding reclamation, device attention, sampler or model support. |
