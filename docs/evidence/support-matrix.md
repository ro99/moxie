...
| none | host-reference synthetic | <=256 requested context; no checkpoint | Shared generation service and diagnostic CLI | **implemented task 0015; owner review pending** | `G-GENERATION-HOST`, `G-GENERATION-ALLOC`, `G-HOST-ARCH` | Typed pull events, chunked prefill, one-token decode, accepted sampler and paged state; exact dense-reference parity for two shapes, cleanup on finish/cancel/failure/drop. No tokenizer, text stop/EOS, HTTP, checkpoint, GPU attention or production CPU fallback. |
