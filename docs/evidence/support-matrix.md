...
| `G-HOST-TEST` | `cargo test --workspace --locked` | host | passed, 457 unit/integration tests + 6 doctests |
...
| `G-HOST-NODRIVER` | host build with `CUDA_HOME=/nonexistent NVCC=/nonexistent`, no CUDA on `PATH` | host | passed, 457 unit/integration tests + 6 doctests, `ldd` shows no `libcuda` |
