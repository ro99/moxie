...
| **inspected, not executed** — `/fast/models/cyankiwi/gemma-4-31B-it-AWQ-8bit` rev `34ca187d` | n/a — host import | n/a | compressed-tensors `pack-quantized` INT8/INT4 **import** to canonical affine form | passed | `G-CT-IMPORT` | **Import only. This is not model support and nothing executes.** Symmetric sources; asymmetric is an explicit typed refusal because no pinned exporter or local artifact establishes its zero-point serialization. The lane order within a packed word is taken from the pinned reader and **cannot** be verified against this artifact, because all four lanes fall inside one scale group — closing it needs paired output against the released model (**O2**). No quality claim follows. Nothing was written under any checkpoint root (**O5**). |
| **inspected, not executed** — `/fast/models/cyankiwi/gemma-4-31B-it-AWQ-8bit` rev `34ca187d` | n/a — host import | n/a | compressed-tensors integer import | passed, task 0018 | `G-CT-IMPORT` | Symmetric `pack-quantized` INT8/INT4 import to canonical affine form. **Import is not execution**: there is no W8A16 or W4A16 path, no repack and no manifest write. Asymmetric sources are an explicit typed refusal. |
