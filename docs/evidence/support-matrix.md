...
| Gate | Command | Lane | Result 2026-09-09 |
|---|---|---|---|
...
| `G-HOST-TEST` | `cargo test --workspace --locked` | host | passed, 523 unit/integration tests + 7 doctests |
...
| `G-HOST-NODRIVER` | host build with `CUDA_HOME=/nonexistent NVCC=/nonexistent`, no CUDA on `PATH`; **`cargo build -p xtask` before `ldd`** | host | passed, 523 unit/integration tests + 7 doctests, `ldd` shows no `libcuda`. The explicit build is load-bearing: without it `ldd` can read a device-lane artifact left in the shared `target/`, which it did once on 2026-09-08 ([toolchain.md](toolchain.md)) |
...
| `G-LEASE-FAULTS` | `cargo test -p moxie-executor --features driver --test driver_faults -- --nocapture` | device | passed, actual allocation/copy/record/free boundary faults on all 3 UUIDs; test-only symbol interposition |
| `G-GPU-ALL` | `cargo xtask-cuda test-gpu` | device | passed, 30 cases, both architectures qualified |
...
Counts are from 2026-09-09 after [task 0009](../tasks/0009-m1-event-backed-leases.md); the M0
...
| none | n/a | n/a | Oversized host/disk weight streaming | **not implemented** | — | M2. The memory authority's accounting core exists (row below) and event-backed leases exist ([task 0009](../tasks/0009-m1-event-backed-leases.md)), but nothing else does: no residency lifecycle, no host cache, no upload readiness, no allocator and no eviction. |
...
- A passing capacity gate means this machine and three GPUs answered a question about themselves and
  the ledger admitted a plan against the answers. It is not evidence that a byte was allocated
  anywhere, that a plan was executed, or that either figure will still hold a second later. The host
  figure in particular is `MemAvailable`, which is the kernel's own estimate and includes cache it
  expects to be able to reclaim.
- A passing ledger gate means the accounting arithmetic and the refusal rules are right for byte
  counts a test or a driver supplied. It is not evidence that a byte was allocated or that a plan exists to
  execute. Both tiers are measured as of task 0008; **nothing spends what they measure.**
