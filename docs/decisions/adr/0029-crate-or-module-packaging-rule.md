...
- Current workspace against the rule: all nineteen crates plus `xtask` inventoried in the README table fixed alongside; **seven** single-production-consumer crates (`interp`, `plan`, `cuda`, `kernels`, `sampling`, `engine`, `models`) each name their confinement property in 09 §G. `moxie-oracles` was listed among them and is not one: `moxie-interp` and `moxie-cli` both depend on it in their `[dependencies]` tables. It satisfies the rule through the **multiple-consumer** clause instead, which is a different argument for the same conclusion, and the audit sentence is corrected rather than the outcome. Independent review caught it; the check is one pass over `crates/*/Cargo.toml` counting `[dependencies]` entries per crate, and it is worth re-running whenever this list is quoted. `moxie-executor` is the execution owner whose engine integration is pending M3 work, not a packaging question.
