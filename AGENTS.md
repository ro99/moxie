...
0044](docs/tasks/0044-m4-cow-paged-fork.md) **accepted** (owner, 2026-09-21):
`PagedSequence::fork` (`crates/moxie-state/src/paged.rs`) qualified for host
...
| Accepted M4.1 | Common paged device attention and complete device state — [roadmap](docs/spec/06-implementation-roadmap.md) `M4.1` ([ADR0033](docs/decisions/adr/0033-number-roadmap-deliverables.md) numbers the roadmap's own deliverable bullets to match this table's labels). [Task 0037](docs/tasks/0037-m4-paged-device-attention.md) **accepted and closed** (owner, 2026-09-20): the BF16 paged kernel, its binding and the 32,768-actual-row gate qualified on both SM86 GPUs and SM120 ([ADR0032](docs/decisions/adr/0032-first-paged-attention-kernel-is-written-here.md) records why no upstream source was adopted); its two remaining items moved to task 0038's acceptance 7 and 8 rather than close by assertion. [Task 0038](docs/tasks/0038-m4-device-kv-state-authority.md) **accepted and closed** (owner, 2026-09-20): `moxie-state` owns device page placement, retention and lifecycle; selected attention plans execute through their admitted query/output slots; the combined wrapped lifecycle (append/attend/abort/truncate/reappend with a reclaimed base, then lower and execute) passes on both SM86 GPUs and SM120; the final mutation battery catches 11 of 11 substitutions, and the measured allocator lane bounds 32 decode steps and catches its deliberate leak. M4.2 (MLA/GLM-5.2 descriptors), M4.3 (host-backed state streaming), M4.4 (COW forks and snapshot/replay) and M4.5 (prefix reuse) are unstarted. The [M3→M4 handover](docs/handovers/2026-09-19-m3-closure-to-m4.md) carries the milestone boundary. |
...
- **Moxie runs downloaded checkpoints as they are** ([ADR 0038](docs/decisions/adr/0038-run-source-checkpoints-directly.md), owner, 2026-09-25). Source layouts are interpreted at load time by the shared importers. No runtime path may require a canonical artifact, and `moxie-repack` is an optional tool.
...
- **A tensor-parallel reduction is exact** (owner, 2026-09-22; [ADR 0036](docs/decisions/adr/0036-tp-reductions-are-exact-by-declared-order.md)). The accumulation order is part of the contract: FP32 partials per shard, combined in FP32 in a declared order, rounded to BF16 once. The single-rank path runs the same order, so TP must be bit-identical to it. There is no tolerance.
