...
| Implemented, reviewed five times, **one clause open** | [Task 0028](docs/tasks/0028-m3-shared-w4a16-w8a16-execution.md) — M3 item 3's **dense** W4A16/W8A16 path. One shared kernel, four catalogue identities, no whole-tensor dequantization; the republished Laguna module executes on all three GPUs within the predeclared threshold. Seven findings, then three of those seven, then two of those three, then one — each round found the repair had closed the path the reproduction used and left the path beside it. **Worked by [task 0029](docs/tasks/0029-allocation-fallible-admission-vocabulary.md), not closed:** the admission path refuses at every position its sweeps reach — **at least 28 of 41** on real hardware — and no refusal leaves a ledger counter charged or a free list moved. **Acceptance clauses 1, 2 and 7 are open**: the rejection and relocation paths have no sweep, and the positions at and after `Rc::new` cannot be swept because it aborts. One item is an owner decision — an API narrowing the task says is the owner's call. Task 0028's repairs inside `admit` stay **fixed but unmeasured**. **Dense only, synthetic activations, no timing.** |
| Next | Task 0029's open clauses — the unswept rejection and relocation paths, and `Arena::release`'s infallible insert — plus two **owner decisions** in its Result section: `Rc::new` has no fallible form, so the arena's shared core still aborts (hand-rolled refcount, a lifetime change, or accept it); and `Label` no longer accepts a non-`'static` borrow, which is an API narrowing the contract says is the owner's call. Then M3 item 3's **expert** half — quantized MoE. Nothing routed executes a quantized weight, so no quantized checkpoint runs as a model. |
| Continuation | [The task 0029 handover](docs/handovers/2026-09-15-task0029-fallible-admission.md); [the task 0028 handover](docs/handovers/2026-09-14-task0028-shared-quantized-execution.md); [the task 0026 handover](docs/handovers/2026-09-14-task0026-safetensors-publication.md) carries task 0025's forward |
