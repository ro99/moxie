...
| Implemented, reviewed once | [Task 0028](docs/tasks/0028-m3-shared-w4a16-w8a16-execution.md) — M3 item 3's **dense** W4A16/W8A16 path. One shared kernel, four catalogue identities, no whole-tensor dequantization; the republished Laguna module executes on all three GPUs within the predeclared threshold. One review found **seven** issues, five P1 — all reproduced, all fixed with a mutation each, none disputed; three of them were in `moxie-repack`'s user surface. **Dense only, synthetic activations, no timing.** |
