...
| Implemented, reviewed twice | [Task 0028](docs/tasks/0028-m3-shared-w4a16-w8a16-execution.md) — M3 item 3's **dense** W4A16/W8A16 path. One shared kernel, four catalogue identities, no whole-tensor dequantization; the republished Laguna module executes on all three GPUs within the predeclared threshold. The first review found **seven** issues, five P1; the re-review found **three of those seven still open** and they are now fixed, each with a mutation in the `T0028` battery. **Dense only, synthetic activations, no timing.** |
