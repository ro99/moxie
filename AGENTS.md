...
| Implemented, reviewed four times, **one finding open** | [Task 0028](docs/tasks/0028-m3-shared-w4a16-w8a16-execution.md) — M3 item 3's **dense** W4A16/W8A16 path. One shared kernel, four catalogue identities, no whole-tensor dequantization; the republished Laguna module executes on all three GPUs within the predeclared threshold. Seven findings, then three of those seven, then two of those three, then one — each round found the repair had closed the path the reproduction used and left the path beside it. **Still open and measured failing:** allocation failure in `moxie-memory`'s shared `PlanRequest` aborts the process, which [task 0029](docs/tasks/0029-allocation-fallible-admission-vocabulary.md) fixes. **Dense only, synthetic activations, no timing.** |
| Next | [Task 0029](docs/tasks/0029-allocation-fallible-admission-vocabulary.md) — the admission vocabulary aborts on a failed allocation, which is task 0028's one open finding and a shared-owner change. Then M3 item 3's **expert** half — quantized MoE. Nothing routed executes a quantized weight, so no quantized checkpoint runs as a model. |
