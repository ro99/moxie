...
| Active M4.1 | Common paged device attention and complete device state — roadmap M4 deliverable 1 of 5. [Task 0037](docs/tasks/0037-m4-paged-device-attention.md) **landed**: the BF16 paged kernel, its binding and the 32,768-actual-row gate qualified on both SM86 GPUs and SM120 ([ADR0032](docs/decisions/adr/0032-first-paged-attention-kernel-is-written-here.md) records why no upstream source was adopted). [Task 0038](docs/tasks/0038-m4-device-kv-state-authority.md) **active**: `moxie-state` owns the device pages and `moxie-plan` lowers an attention node to device handles, but no lowered plan executes through its own arena slots and acceptance criterion 2 is unmet. M4.2 (MLA/GLM-5.2 descriptors), M4.3 (host-backed state streaming), M4.4 (COW forks and snapshot/replay) and M4.5 (prefix reuse) are unstarted. The [M3→M4 handover](docs/handovers/2026-09-19-m3-closure-to-m4.md) carries the milestone boundary. |
