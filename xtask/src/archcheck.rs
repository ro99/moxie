...
fn allowlist() -> BTreeMap<&'static str, Allowed> {
    ...
    BTreeMap::from([
        ...
        // The offline repacker (ADR 0021/0022, amended by ADR 0024): the one
        // program that writes a canonical artifact. The writer is a module of
        // it rather than a crate, so "who may publish" needs no rule at all --
        // nothing else can name what is not in its dependency tree.
        (
            "moxie-repack",
            Allowed {
                workspace: &[
