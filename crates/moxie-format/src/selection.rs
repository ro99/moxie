...
/// The version a **generated plan** carries: the compact form of the same
/// selection, which expands into exactly the structure version 1 spells out.
///
/// Version 1 remains readable. A hand-written selection is still a selection;
/// what version 2 adds is a representation a whole model fits in. See
/// [ADR 0026].
///
...
pub const MAX_SELECTED_TENSORS: usize = 262_144;
...
pub fn parse(text: &str) -> Result<Selection> {
    ...
    if v.version == PLAN_VERSION {
        // The compact form, expanded here into the same structure below. One
        // parser and one preflight for both spellings: a generated plan that
        // took a different path through validation would be a second contract.
        return crate::plan::expand(text);
    }
    if v.version != SELECTION_VERSION {
        return Err(invalid(format_args!(
            "selection is version {}, this repacker accepts version {SELECTION_VERSION} \
             (hand-written) and version {PLAN_VERSION} (generated by `moxie-repack plan`)",
            v.version
        )));
    }
...
"#;
    ...
    fn every_refusal_names_what_it_refused() {
        for (text, needle) in [
            (
                // Version 2 is now the generated **plan**, so an unknown
                // version has to be a genuinely unknown one.
                with(PQ).replace("version = 1", "version = 3"),
                "accepts version 1",
            ),
