...
// probes, not GPU marketing names". `m0_axpy_f32` is the launch probe.

#include <cuda_bf16.h>

extern "C" __global__ void moxie_m0_axpy_f32(const float* x, float* y, float a, unsigned int n) {
...
extern "C" __global__ void moxie_m0_f32_to_bf16_bits(const float* src, unsigned short* dst, unsigned int n) {
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
